...
Write-Info "[INFO] Cargando configuracion desde .env..."
...
Write-Info "[INFO] Verificando instalacion de PostgreSQL..."
$psqlPath = Get-Command psql -ErrorAction SilentlyContinue
if (-not $psqlPath) {
    Write-Error "[ERROR] PostgreSQL no esta instalado o no esta en el PATH"
    Write-Warning "Por favor instala PostgreSQL desde: https://www.postgresql.org/download/windows/"
    exit 1
}
Write-Success "[OK] PostgreSQL encontrado: $($psqlPath.Source)"
Write-Host ""
...
# Test connection
Write-Info "[INFO] Probando conexion a PostgreSQL..."
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "  - PostgreSQL esta ejecutandose"
...
Write-Info "[INFO] Verificando si la base de datos existe..."
$dbExists = & psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d postgres -tAc "SELECT 1 FROM pg_database WHERE datname='$DB_NAME';" 2>&1

if ($dbExists -eq "1") {
    Write-Warning "[WARNING] La base de datos '$DB_NAME' ya existe"
} else {
    Write-Info "[INFO] Creando base de datos '$DB_NAME'..."
...
# Run migrations
Write-Info "[INFO] Ejecutando migraciones..."
...
if ($migrations.Count -eq 0) {
    Write-Warning "[WARNING] No se encontraron archivos de migracion"
} else {
    foreach ($migration in $migrations) {
        Write-Info "  Ejecutando: $($migration.Name)..."
        $result = & psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME -f $migration.FullName 2>&1
        if ($LASTEXITCODE -ne 0) {
            Write-Error "    [ERROR] Error en migracion: $($migration.Name)"
            Write-Host $result
            exit 1
        }
        Write-Success "    [OK] Completada"
    }
    Write-Success "[OK] Todas las migraciones ejecutadas exitosamente"
}
Write-Host ""

# Seed data if requested
if ($Seed) {
    Write-Info "[INFO] Insertando datos de prueba..."
    $seedFile = ".\scripts\seed-data.sql"
    
    if (Test-Path $seedFile) {
        $result = & psql -h $DB_HOST -p $DB_PORT -U $DB_USER -d $DB_NAME -f $seedFile 2>&1
        if ($LASTEXITCODE -ne 0) {
            Write-Warning "[WARNING] Error al insertar datos de prueba (puede ser normal si ya existen)"
        } else {
            Write-Success "[OK] Datos de prueba insertados"
            Write-Host ""
            Write-Info "[INFO] Datos de prueba disponibles:"
            ...
            Write-Host "  - Sistema informatico configurado"
        }
    } else {
        Write-Warning "[WARNING] Archivo de datos de prueba no encontrado: $seedFile"
    }
    Write-Host ""
}

# Verify tables
Write-Info "[INFO] Verificando tablas creadas..."
...
if ($LASTEXITCODE -eq 0) {
    Write-Success "[OK] Tablas encontradas: $tableCount"
} else {
    Write-Warning "[WARNING] No se pudo verificar las tablas"
}

Write-Host ""
Write-Success "================================================"
Write-Success "  [OK] Setup completado exitosamente"
Write-Success "================================================"
Write-Host ""
